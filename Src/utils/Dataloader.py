...
class test_dataset1:
    ...
    def load_data(self):
        print(f"Loading data: Index={self.index}, Size={self.size}")
        image = self.rgb_loader(self.images[self.index])
        image = self.transform(image).unsqueeze(0)
        if not os.path.exists(self.images[self.index]):
            raise FileNotFoundError(f"Image file not found: {self.images[self.index]}")
        if not os.path.exists(self.gts[self.index]):
            raise FileNotFoundError(f"GT file not found: {self.gts[self.index]}")
...
class test_dataset:
    ...
    def load_data(self):
        print(f"Loading data at index: {self.index} / {self.size}")
        image = self.rgb_loader(self.images[self.index])
        image = self.transform(image).unsqueeze(0)
        ...
        self.index += 1
        self.index = self.index % self.size
        assert len(self.gts) == len(self.images), "Length mismatch between images and ground truths!"
        print(f"Next index: {self.index}")  # Debugging next index
